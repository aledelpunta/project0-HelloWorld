...
public class MagicHat {
    public static void main(String[] args) throws InterruptedException{
        ...
        final int REST          = students.length % NUM_HOUSES; //% per il resto della divisione
        ...
        Random hat  = new Random(); //random viene istanziato
        int destiny;

        for(int i = 0; i < (students.length - REST); i++){
           // boolean assigned = false;
            System.out.printf("lo studente %s finirà in....",students[i]);//%s parametro tipo string
            Thread.sleep(3000);
            String house = null;
            do{
                destiny = hat.nextInt(NUM_HOUSES);     //hat.nextInt mi genera un valore casuale compreso tra 0 e 3 cioe il valore di NUM_HOUSES-1
                switch(destiny){
                    case 0:                             //if(destiny == 0){}
                        if(iG < (DIM_HOUSES - 1)){
                            gryffindor[iG] = students[i]; //assume il valore della posizione i
                            iG++;
                            //assigned = true;
                            house = "GRYFFINDOR";
                        }
        ...
        for(int i = (students.length - REST); i < students.length; i++){
            //destiny = hat.nextInt(NUM_HOUSES);  //questo non serve
           // boolean assigned = false;
           System.out.printf("lo studente %s finirà in....",students[i]);
           Thread.sleep(3000);
           String house = null;
            do{
